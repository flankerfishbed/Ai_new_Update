...
class ExPASyIntegration:
    ...
    def _parse_protparam_response(self, html_content: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Extract molecular weight with multiple patterns
            mw_patterns = [
            ...
            for pattern in mw_patterns:
                mw_match = re.search(pattern, html_content, re.IGNORECASE)
                if mw_match:
                    results['molecular_weight'] = float(mw_match.group(1))
                    break
            
            # Extract isoelectric point with multiple patterns
            pi_patterns = [
                r'Theoretical pI:\s*([\d.]+)',
                r'Isoelectric point:\s*([\d.]+)',
                r'pI:\s*([\d.]+)',
                r'pI\s*([\d.]+)'
            ]
            
            for pattern in pi_patterns:
                pi_match = re.search(pattern, html_content, re.IGNORECASE)
                if pi_match:
            ...
            # Extract instability index with multiple patterns
            ...
            # Extract aliphatic index with multiple patterns
            aliphatic_patterns = [
            ...
            # Extract GRAVY (Grand Average of Hydropathy) with multiple patterns
            gravy_patterns = [
                r'Grand average of hydropathy \(GRAVY\):\s*([-\d.]+)',
                r'GRAVY:\s*([-\d.]+)',
                r'Grand average of hydropathy:\s*([-\d.]+)',
                r'GRAVY\s*([-\d.]+)',
                r'GRAVY \(Grand average of hydropathy\):\s*([-\d.]+)'
            ]
            
            for pattern in gravy_patterns:
                gravy_match = re.search(pattern, html_content, re.IGNORECASE)
                if gravy_match:
    ...
    def _calculate_stability_metrics(self, expasy_data: Dict[str, Any], sequence: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Fallback calculations if ExPASy parsing failed
            molecular_weight = expasy_data.get('molecular_weight', 0)
            if molecular_weight == 0:
                molecular_weight = self._calculate_molecular_weight(sequence)
                st.info("💡 Using calculated molecular weight (ExPASy parsing failed)")
            
            if gravy_score == 0:
                gravy_score = self._calculate_gravy_score(sequence)
                st.info("💡 Using calculated GRAVY score (ExPASy parsing failed)")
