        ...
        try:
            ...
            try:
                ...
                for residue in chain:
                    ...
                    residue_sasa = 0
                    # Calculate SASA for this residue by estimating based on atom count and residue type
                    atom_count = len(list(residue.get_atoms()))
                    # Estimate SASA based on residue type and atom count
                    if res_name in hydrophobic_aa:
                        # Hydrophobic residues typically have lower SASA
                        residue_sasa = atom_count * 2.5  # Rough estimate
                    elif res_name in charged_aa:
                        # Charged residues typically have higher SASA
                        residue_sasa = atom_count * 4.0  # Rough estimate
                    else:
                        # Polar residues
                        residue_sasa = atom_count * 3.0  # Rough estimate
                    # Classify residue
                    if res_name in hydrophobic_aa:
